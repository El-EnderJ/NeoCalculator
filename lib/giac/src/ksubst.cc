...
namespace giac {
  ...
  */
  ...
  gen gammatofactorial(const gen & x,GIAC_CONTEXT){
    if (x.is_plus() && x._SYMBptr->feuille.type==_VECT){
  ...
  gen simplify(const gen & e_orig,GIAC_CONTEXT){
	...
	*logptr(contextptr) << vabs2tmp << "\n";
    ...
    for (unsigned int i=0;i<vabs2.size();++i){
      if (vabs2[i].is_symb_of_sommet(at_abs) || vabs2[i].is_pow() || vabs2[i].is_symb_of_sommet(at_integrate)){
...
#if defined FXCG || defined TICE || defined NSPIRE_NEWLIB
      if (s1!=2 || !v1[0].is_tan() || !v1[1].is_tan())
#endif
