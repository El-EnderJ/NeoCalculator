...
namespace giac {
  ...
  */
  ...
  static void Muldense_POLY1(const modpoly::const_iterator & ita0,const modpoly::const_iterator & ita_end,const modpoly::const_iterator & itb0,const modpoly::const_iterator & itb_end,environment * env,modpoly & new_coord,int taille){
	...
	add_mul(&res->z,prod,*ita_cur,*itb_cur);
        // res = res + longlong(*ita_cur) * (*itb_cur);
      }
      if (env && env->moduloon){
  ...
  static void Mulmodpolysmall(modpoly::const_iterator & ita,modpoly::const_iterator & ita_end,modpoly::const_iterator & itb,modpoly::const_iterator & itb_end,environment * env,modpoly & new_coord){
    ...
    if (
        ...
        1
#else
        ( a <= INT_KARAMUL_SIZE) || ( b <= INT_KARAMUL_SIZE)
#endif
        ){
  ...
  void mulmodpoly(const modpoly & a, const modpoly & b, environment * env,modpoly & new_coord){
    ...
    if (
        env && (env->moduloon) && is_zero(env->coeff) && !env->complexe && (env->modulo.type==_INT_) && (env->modulo.val < smallint) && (product_deg < 256)
         )
	...
	itb=b.begin();
      ...
      if (
          ...
          1
#else
          (as<=KARAMUL_SIZE) && (bs<=KARAMUL_SIZE)
#endif
          )
	Muldense_POLY1(ita,ita_end,itb,itb_end,env,new_coord,taille);
      else
  ...
  void operator_times (const modpoly & a, const modpoly & b,environment * env,modpoly & new_coord) {
	...
	vreverse(af.begin(),af.end()); af.resize(n);
	vreverse(bf.begin(),bf.end()); bf.resize(n);
    ...
    if (0 && ita==ita_end && itb==itb_end){ // integer coefficients
      mulmodpoly(a,b,env,new_coord);
      return;
    }
  ...
  bool DivRem(const modpoly & th, const modpoly & other, environment * env,modpoly & quo, modpoly & rem,bool allowrational){
    ...
    bool invother=false;
#if 0
    if (coeff.type==_USER){ invother=true;invcoeff=inv(coeff,context0); }
    if (coeff.type==_EXT){
      gen coeff0=*coeff._EXTptr;
      if (coeff0.type==_VECT){
	for (int i=0;i<coeff0._VECTptr->size();++i){
	  if ((*coeff0._VECTptr)[i].type==_USER){invcoeff=inv(coeff,context0); invother=true; break; }
	}
      }
    }
#endif
    ...
    */
	...
	if (env && env->moduloon)
	  ...
	  q=*tmpend;
        ...
        //dbg_printf("divrem loop %s div %s\n",tmpend->print(context0).c_str(),coeff.print(context0).c_str());
	...
	if (!allowrational){
      ...
      //dbg_printf("divrem loop a=%i b=%i q=%s\n",a,b,q.print(context0).c_str());
      quo.push_back(q);
      --tmpend;
      if (!is_zero(q)) {
	// tmp <- tmp - q *B.shifted
  ...
  dense_POLY1 AscPowDivRemModifiable(dense_POLY1 & num, dense_POLY1 & den,int order){
    ...
    vreverse(den.begin(),den.end());
    rrdm(den,order);
    // reverse and adjust num degree to 2*order
    vreverse(num.begin(),num.end());
    rrdm(num,2*order);
    dense_POLY1 quo,rem;
  ...
  */
  ...
  void gcdsmallmodpoly(const modpoly &p,const modpoly & q,int m,modpoly & d){
...
#if defined VISUALC || defined BESTA_OS || defined TICE
    ...
    //dbg_printf("gcdsmallmod %s %s %s %i\n",gen(p).print(context0).c_str(),gen(q).print(context0).c_str(),gen(d).print(context0).c_str(),m);
#if defined VISUALC || defined BESTA_OS || defined TICE
    delete [] asave;
    delete [] bsave;
#endif
  ...
  bool gcdmodpoly(const modpoly &p,const modpoly & q,environment * env,modpoly &a){
    // dbg_printf("gcdmod mod %s start %s %s \n",env?env->modulo.print(context0).c_str():"0",gen(p).print(context0).c_str(),gen(q).print(context0).c_str());
    if (!env){
#ifndef NO_STDEXCEPT
      setsizeerr();
#endif
    ...
    while (!b.empty()){
      gen s=b.front();
      //dbg_printf("gcd loop %s %s\n",gen(a).print(context0).c_str(),gen(b).print(context0).c_str());
    ...
    //dbg_printf("gcdmod end %s %s %s\n",gen(p).print(context0).c_str(),gen(q).print(context0).c_str(),gen(a).print(context0).c_str());
    return true;
  }
  ...
  */
  ...
  void subresultant(const modpoly & P,const modpoly & Q,gen & res){
    ...
    for (int i=0;i<P.size();++i){
      gen g=P[i];
      if (g.type==_USER){ res=gf_ext_resultant(P,Q); return; }
      if (g.type==_EXT){
	gen h=*g._EXTptr;
	if (h.type==_VECT){
	  for (int j=0;j<h._VECTptr->size();++j){
	    gen k=(*h._VECTptr)[j];
	    if (k.type==_USER){ res=gf_ext_resultant(P,Q); return;}
	  }
	}
      }
    }
  ...
  */
  ...
  bool giac_gcd_modular_algo1(polynome &p,polynome &q,polynome &d){
    ...
    if (is_undef(pp) || is_undef(qq))
      return false;
    //dbg_printf("gcd_modular_algo1 1\n");
	...
	if (env.complexe){
      ...
      //dbg_printf("gcd_modular_algo1 2\n");
      if (is_undef(gcdmod))
	return false;
      ...
      //dbg_printf("gcd_modular_algo1 3\n");
	...
	if (productmodulo==gen(1)){ // no need to combine primes
      ...
      //dbg_printf("gcd_modular_algo1 3 currentgcd=%s\n",gen(currentgcd).print(context0).c_str());
      // check candidate gcd
      modpoly dmod(modularize(currentgcd,productmodulo,&envtmp));
      //dbg_printf("gcd_modular_algo1 dmod=%s\n",gen(dmod).print(context0).c_str());
      if (is_undef(dmod))
	return false;
      ppz(dmod);
      //dbg_printf("gcd_modular_algo1 3 ppz dmod=%s\n",gen(dmod).print(context0).c_str());
      if ( (DenseDivRem(pp,dmod,p_simp,rem,true)) && (rem.empty()) && (DenseDivRem(qq,dmod,q_simp,rem,true)) && (rem.empty()) ){
        //dbg_printf("gcd_modular_algo1 4 ppz p_simp=%s\n",gen(p_simp).print(context0).c_str());
	p=unmodularize(p_simp);
        //dbg_printf("gcd_modular_algo1 unmod q\n");
	q=unmodularize(q_simp);
        //dbg_printf("gcd_modular_algo1 unmod d\n");
	d=unmodularize(dmod);
        //dbg_printf("gcd_modular_algo1 end\n");
	return true;
      }
      //dbg_printf("gcd_modular_algo1 4\n");
    }
    return false;
  }
