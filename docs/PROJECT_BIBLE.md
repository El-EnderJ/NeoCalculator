...
> **Lenguaje**: C++17 · **CAS-Lite Engine**: Producción activa
...
3. [CAS-Lite Engine — Arquitectura Interna](#3-cas-lite-engine--arquitectura-interna)
...
10. [Cómo Ampliar el CAS-Lite](#10-cómo-ampliar-el-cas-lite)
...
│  └──────────────┘  └──────────────────┘  └────────────────────────┘   │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │           EquationsApp (CAS-Lite UI)  ★ NUEVO                   │  │
│  │  States: SELECT → EQ_INPUT → RESULT → STEPS                     │  │
│  │  Pipeline: ExprNode → ASTFlattener → Solver → SymToAST          │  │
│  └──────────────────────────────────────────────────────────────────┘  │
...
└────────────────────────────────────────────────────────────────────────┘
       ...
       ▼                       ▼                        ▼
┌─────────────┐   ┌────────────────────┐   ┌───────────────────────┐
│ Math Engine │   │  CAS-Lite Engine   │   │    Hardware Layer     │
│             │   │                    │   │                       │
│ Tokenizer   │   │ PSRAMAllocator<T>  │   │ DisplayDriver         │
│ Parser      │   │ SymPoly + Rational │   │ (TFT_eSPI FSPI)       │
│ Evaluator   │   │ ASTFlattener       │   │ lvglFlushCb DMA       │
│ ExprNode    │   │ SingleSolver       │   │ KeyMatrix 6×8         │
│ VarContext  │   │ SystemSolver       │   │ SerialBridge          │
│ EqSolver    │   │ CASStepLogger      │   │ LvglKeypad (indev)    │
└─────────────┘   │ SymToAST           │   │ LittleFS              │
                  └────────────────────┘   └───────────────────────┘
```
...
**Apps LVGL-nativas**: `CalculationApp`, `GrapherApp`, `EquationsApp`, `MainMenu` → `g_lvglActive = true`.  
...
## 3. CAS-Lite Engine — Arquitectura Interna

El CAS-Lite es un motor de álgebra simbólica propio, diseñado para restricciones embedded: sin heap ilimitado, sin STL pesado, sin dependencias externas. Todo vive en PSRAM.
...
├── SymPoly.h/.cpp        ← Rational (fracción exacta), SymPoly (coefs Rational)
...
| `ASTFlattener` | `math/cas/ASTFlattener.h/.cpp` | `ExprNode` → `SymPoly` |
| `SingleSolver` | `math/cas/SingleSolver.h/.cpp` | Ecuación 1 var (L/Q/N-R) con pasos |
| `SystemSolver` | `math/cas/SystemSolver.h/.cpp` | Sistema 2×2 gaussiana con pasos |
| `CASStepLogger` | `math/cas/CASStepLogger.h/.cpp` | `StepVec` PSRAM — 4 tipos de paso |
| `SymToAST` | `math/cas/SymToAST.h/.cpp` | `Rational`/`SolveResult` → `ExprNode` |
...
| `EquationsApp` | `apps/EquationsApp.cpp/.h` | ✅ | CAS-Lite: 1-var, 2×2, pasos PSRAM |
| Sequences, Statistics, Regression | — | 🔲 | Pendiente Fase 6 |
| Table, Probability, Settings | — | 🔲 | Pendiente Fase 6 |
...
|:--------|:------:|:------------|
| `tests/CASTest.h/.cpp` | ✅ | 53 tests CAS-Lite (Fases A-D) |
...
- ✅ **CAS-Lite Engine** — SymPoly, Flattener, SingleSolver, SystemSolver, Logger, SymToAST
- ✅ **EquationsApp** — 4 estados, modos 1-var y 2×2, pasos PSRAM
- ✅ **53 tests CAS** — todos passing (desactivados en producción)

### Build Stats

| Recurso | Usado | Total | % |
|:--------|------:|------:|:-:|
| RAM | 94 920 B | 327 680 B | **29.0%** |
| Flash | 1 118 121 B | 6 553 600 B | **17.1%** |

### Pendiente

- ⏳ Teclado físico — resolver conflicto GPIO 4/5 (TFT DC/RST vs ROW3/ROW4)
- ⏳ 7 apps con lógica: Statistics, Regression, Sequences, Table, Probability, Settings
- ⏳ CAS avanzado: derivadas, integrales, matrices
...
```cpp
// En SymPoly.h, derivative() existe para polinomios.
// Para funciones transcendentes, crear SymFunction:
// d/dx(sin(u)) = cos(u) * u'
// d/dx(e^u)   = e^u * u'
// Requiere extensión del AST de ExprNode para funciones compuestas.
```
...
*Documentación maestra — última actualización: Febrero 2026*
