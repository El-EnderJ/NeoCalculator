...
typedef long pid_t;
#else // VISUALC
#if !defined(__MINGW_H) && !defined(NSPIRE) && !defined(FXCG) && !defined(__ANDROID__) && !defined(NSPIRE_NEWLIB) && !defined(OSX) && !defined(IOS) && !defined(OSXIOS) && !defined(FIR_LINUX) && !defined TICE
// wince_replacements.h provides compatibility for Windows CE builds.
// It's not available/needed on embedded targets like ESP32; disable it here.
#if 0
...
#include <stdexcept>
#include "help.h"
#if !defined(FXCG) && !defined TICE// !defined(GIAC_HAS_STO_38) // && !defined(ConnectivityKit) && !defined(BESTA_OS)
#include "tinymt32.h"
#endif
...
#ifdef TICE
  extern const int debug_infolevel;
#else
