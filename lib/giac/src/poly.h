  ...
  */
  ...
  bool tensor<T>::TDivRemHash(const tensor<T> & b,tensor<T> & quo,tensor<T> & r,bool allowrational,int exactquo,double qmax) const {
	...
	if (hashdivrem<T,unsigned>(p1,p2,quot,remain,vars,/* reduce */0,qmax,allowrational)){
	  ...
	  ans=1e38; // don't make another unsuccessfull division!
      }
      if (ans/RAND_MAX<RAND_MAX){
  ...
  bool tensor<T>::TPseudoDivRem (const tensor<T> & other, tensor<T> & quo, tensor<T> & rem, tensor<T> & a) const {
    //dbg_printf("TPseudo %s by %s\n",this->dbgprint(),other.dbgprint());
...
#else
      tensor<T> tmp1(rem*b0);
      tensor<T> tmp2(rem0*other);
      rem=tmp1-tmp2;
      //dbg_printf("tpseudodivrem sub (%s)-(%s)\n",tmp1.dbgprint(),tmp2.dbgprint());
#endif
      //dbg_printf("TPseudo Result=%s\n",rem.dbgprint());
    }
    return true;
  }
  ...
  void Tcontentgcd(const tensor<T> &p, const tensor<T> & q, tensor<T> & cont,tensor<T> & prim,int gcddeg){
    ...
    while (
      ...
      //dbg_printf("Tgcdpsr adeg=%i bdeg=%i a=%s b=%s\n",m,n,a.dbgprint(),b.dbgprint());
	...
	a.TPseudoDivRem(b,quo,r,tmp);
        //dbg_printf("Tgcdpsr r=%s rsize=%i\n",r.dbgprint(),r.coord.size());
    ...
    //dbg_printf("Tgcdpsr end b=%s\n",b.dbgprint());
    b.TDivRem1(Tlgcd(b),prim,r,true);
    //dbg_printf("Tgcdpsr end prim=%s\n",prim.dbgprint());
  }
