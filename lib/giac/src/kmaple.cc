...
namespace giac {
  ...
  gen _about(const gen & g,GIAC_CONTEXT){
    if ( g.type==_STRNG && g.subtype==-1) return  g;
    if (g.type==_VECT)
      return giac::apply(g,contextptr,_about);
    if (g.type==_IDNT)
  ...
  gen _trunc(const gen & args,GIAC_CONTEXT){
	...
	return _trunc(args._VECTptr->back(),contextptr);
      ...
      return giac::apply(args,contextptr,_trunc);
    }
  ...
  gen _evalc(const gen & g,GIAC_CONTEXT){
    if ( g.type==_STRNG && g.subtype==-1) return  g;
    if (g.type==_VECT)
      return giac::apply(g,_evalc,contextptr);
    ...
    if (!l.empty()){
      ...
      lp=*giac::apply(gen(lp),contextptr,arg_CPLX)._VECTptr;
...
#define S_IWUSR 00200
  ...
  */
  ...
  static gen crsolve(const gen & f0,const gen &u,const gen & n,vecteur & initcond0,GIAC_CONTEXT){
    ...
    gen f=giac::apply(f0,equal2diff);
    if (n.type!=_IDNT){
